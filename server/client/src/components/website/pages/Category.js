...
function Category({match}) {
  ...
  useEffect(() => {
    ...
    // Get All Business
    fetch("/user/getBusinessForWebAcdCategory/"+match.params.slug, {
      method: "GET",
      headers: {
      ...
      <div className={"container-fluid bg-white section-2"}>
        <div className={"container py-4"}>
          <div className={"row py-3"}>
                ...
                {list.category? <div className={"col-md-6 mt-2"}>
                  <div className="card border-0 shadow rounded-0">
                    <Link to={`/listing/${list.slug}`}>
                        <img
                        src="https://i.insider.com/5da9f73ccc4a0a1d720b7b1a"
            ...
            {!bizCount && <div className={"col-md-8"}>
            <div className={"alert alert-danger rounded-0 h6"}>There is no any Business Available in This Category</div>    
            </div>}
          </div>
        </div>
      </div>
